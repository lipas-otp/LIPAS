...
Start with `00_playground.py`: it is the ordinary-Python, high-level path.
Examples 01–08 deliberately expose lower-level harnesses for readers who want
to understand the runtime beneath it; 09–12 return to high-level Agent,
supervision, Team, and external-operation scenarios.

| Example | Scenario | Provider required |
|---|---|---|
| `00_playground.py` | Easiest Agent + one tool + durable trace | Ollama |
...
| `12_supervised_agent.py` | High-level `Agent(supervisor_policy=...)` | Ollama |
