...
概念。两层能力共同使用 runtime Effect 记录，不维护平行的第二套审计事件模型。
...
面向调用者的 token streaming、自动 lease heartbeat 以及完整任务工作台仍属于路线图
目标。执行状态 store 与 claim/Effect session 有意作为两份独立持久记录；当前 durable
Agent 要求二者都使用 SQLite。

## 交付阶段

### 阶段一：可靠执行纵切

- 在已经交付的 ReAct checkpoint 上增加 lease heartbeat 和 timeout 处理；
- 增加高层模型与工具 streaming；
- 在已经交付的持久 cancellation、审批 interrupt/resume 与 orphan 检测之上增加
  timeout recovery；
- 增加 Task、Workspace、Run、Approval、Artifact 和 Report 应用模型；
- 增加受限文件、Shell 和 Git capability；
- 从 CLI 跑通“检查 → 修改 → 验证 → 报告”；

退出标准：同一个任务在进程终止后可以恢复，不会静默丢失状态或重复已完成的 write。

### 阶段二：CLI Private Alpha

- 增加 HTTP 与 MCP capability；
- 持久化审批 inbox 及审批消费状态；
