...
def _request(text: str) -> Request:
    return Request("fake", [{"role": "user", "content": text}], 128)
...
def _record_one(db_path: str, text: str = "hello") -> None:
    ...
    try:
        ...
        reply = asyncio.run(harness.call(_request("hi")))
        assert reply.content[0]["text"] == text
    finally:
        rowset.store.close()
...
def test_replay_transcript_round_trip(db_dir):
    ...
    with replay(db) as r:
        ...
        harness = LLMHarness(
            adapter=r.stub_adapter(), rowset=r.rowset,
...
def test_replay_stub_adapter_raises_when_cursor_exhausted(db_dir):
    ...
    with replay(db) as r:
        ...
        reply = asyncio.run(harness.call(_request("1")))
        assert reply.content[0]["text"] == "only one"

        # Second call: cursor exhausted → stub raises.
        with pytest.raises(ReplayExhausted):
            asyncio.run(harness.call(_request("2")))
...
def test_replay_re_execute_llm_uses_real_adapter(db_dir):
    ...
    with replay(db, re_execute_llm=True) as r:
        ...
        harness = LLMHarness(adapter=new_adapter, rowset=r.rowset)
        reply = asyncio.run(harness.call(_request("x")))
        assert reply.content[0]["text"] == "re-executed"
