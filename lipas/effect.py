...
Historical names
----------------
Tag string values are preserved as ``"call_intent"`` / ``"call_result"``
/ ``"call_rejected"`` so that pre-P3.0 stores remain readable.  The
Python-side constants are renamed to ``TAG_EFFECT_*`` to reflect the
generalized semantics.  The old ``TAG_CALL_*`` and ``F_CALL_ID`` names
remain as aliases in ``lipas.rows.effect`` and emit no warning; they
are slated for removal in P4.
"""
...
class EffectKind(str, Enum):
    ...
    More kinds may be added in P4+ (e.g. ``HTTP_REQUEST`` for direct
