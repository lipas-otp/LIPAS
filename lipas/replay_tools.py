"""LIPAS · ToolReplayer — RFC-001 (A1 v0.2) implementation.
...
The matrix is normatively defined in RFC-001 §4. This module is the
mechanical translation of that matrix into Python; behavioural changes
require an RFC amendment, not just a code change here.
...
guards / budget gate (RFC-001 §6.2). The decision drives one of four
operations:
...
# =====================================================================
# SideEffectClass lattice (A1 §5.1)
# =====================================================================
...
# =====================================================================
# Error / warning hierarchy (A1 §6.4)
# =====================================================================
...
# Disjoint-subtree assertion (A1 §6.4 normative): no class may inherit
...
class ToolReplayer:
    ...
    def lookup(
        ...
        Match policy (A1 §3.3): exact equality on tool_name AND
    ...
    def decide(
        ...
        """Encode RFC-001 §4 + §3.4 + §5.1 as a pure function.
        ...
        # §5.1 class-mismatch resolution (only when there is a recording).
        # Compares *declared* classes between recorded run and current
        # run.  Must run BEFORE §3.4 obs-only downgrade: obs-only is a
        ...
        # §3.4 observability-only downgrade applied AFTER mismatch
    ...
    def _present_branch(
        ...
        if effective is SideEffectClass.IDEMPOTENT_WRITE:
            # §4.1 footnote 1: idempotency-key inspection is delegated
            # to the tool itself in v1. Downgrade to substitute, the
