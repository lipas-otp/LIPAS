"""
lipas.supervisor — policy-driven supervisor.
...
behaviours pick their own tick site.
...
C1. **Drive site**. ``Supervisor.tick(view, ctx)`` MUST be called by an
    ...
    ToolReplayer) MAY mirror them into a target store; that is not currently
    implemented.
...
C4. **Determinism**. Predicates MUST be pure functions of ``(view, ctx)``
    under the strategy-purity contract (lipas/calculus.py). Supervisor
...
C5. **Goal-blocked pairing**. Every ``supervisor_terminate`` and
...
``fields`` for explicit persisted shape identification. It is currently
informational; future store migrations may use it as an upcast boundary.
"""
...
class RetryRecommendation:
    """One retry recommendation projected from the supervisor claim tape."""
...
class SupervisorProjection:
    """Read-only summary of a Supervisor's recorded recommendations."""
...
def project_supervisor(store: ClaimStore) -> SupervisorProjection:
    """Read Supervisor state using ClaimStore's tag index.

    This is a projection, not a fold strategy: it adds no hidden state or
    behaviour to the execution loop.
    """
    retries = tuple(
        RetryRecommendation(
            target_effect_id=str(claim.fields.get(F_SUP_TARGET_EFFECT_ID, "")),
            idempotency_key=str(claim.fields.get(F_SUP_IDEMPOTENCY_KEY, "")),
            attempt_index=int(claim.fields.get(F_SUP_ATTEMPT_INDEX, 0)),
            max_attempts=int(claim.fields.get(F_SUP_MAX_ATTEMPTS, 0)),
            reason=str(claim.fields.get(F_SUP_REASON, "")),
        )
        for claim in store.filter(tag=TAG_SUPERVISOR_RETRY)
    )
    terminations = store.filter(tag=TAG_SUPERVISOR_TERMINATE)
    escalations = tuple(
        dict(claim.fields)
        for claim in store.filter(tag=TAG_SUPERVISOR_ESCALATE)
    )
    return SupervisorProjection(
        retries=retries,
        terminated=bool(terminations),
        terminate_reason=(
            str(terminations[-1].fields.get(F_SUP_REASON, ""))
