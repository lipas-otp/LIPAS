...
ReActAgent / DeclarativeAgent actually pass and parse. See module
...
from .streaming import Done, StreamEvent
from .usage import Usage

try:
    from .._lipas_meta import strip_meta  # type: ignore
except ImportError:  # pragma: no cover
    def strip_meta(messages: Any) -> Any:  # type: ignore
        return messages
...
class AnthropicAdapter:
    ...
    def _build_params(self, request: Request) -> dict[str, Any]:
        # request.messages is dict-shaped at runtime (see module
        # docstring); strip_meta is idempotent on already-clean dicts.
        messages = strip_meta(list(request.messages))
