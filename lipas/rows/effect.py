"""
Effect Row — the kind-discriminated causal/lineage projection (P3.0).
...
Historical names
----------------
``TAG_CALL_INTENT`` / ``TAG_CALL_RESULT`` / ``TAG_CALL_REJECTED`` /
``F_CALL_ID`` are preserved as deprecated aliases below; new code
should use the ``TAG_EFFECT_*`` / ``F_EFFECT_ID`` names from
``lipas.effect``.
...
# Effect ids may use either "call_" (LLM, historical) or "tool_"
# (tool, P3.0+) prefix.  Both 12-hex-char suffix.  Validators accept
...
# ─── Deprecated aliases (P3.0 → removal in P4) ──────────────────────
# These keep external code that imports the old names compiling.
# Prefer the TAG_EFFECT_* / F_EFFECT_ID names in new code.
TAG_CALL_INTENT   = TAG_EFFECT_INTENT
TAG_CALL_RESULT   = TAG_EFFECT_RESULT
TAG_CALL_REJECTED = TAG_EFFECT_REJECTED
F_CALL_ID         = F_EFFECT_ID
...
class EffectNode:
    ...
    # Back-compat property — pre-P3.0 code used `.call_id`.
    @property
    def call_id(self) -> str:
        return self.effect_id


# Pre-P3.0 alias.  Prefer EffectNode.
CallNode = EffectNode
