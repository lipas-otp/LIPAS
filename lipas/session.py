...
Two helpers:
  ...
  open_session(path, ...) -> RowSet
      ...
      row triple (Effect / History / Capability). Advanced callers plug the
      RowSet into LLMHarness / ToolHarness; ordinary applications use Agent.
  ...
  replay(source_path, ...) -> contextmanager yielding ReplaySession
      ...
      replay_cursor / tool_replayer / target rowset to plug into the
...
def open_session(
    ...
    *,
    ...
    Examples
    --------
        agent = Agent(adapter=adapter, session_path="session.db")
        try:
            result = await agent("summarize this")
        finally:
            agent.close()
...
class ReplayStubAdapter:
    ...
    Plugs into ``LLMHarness(adapter=...)`` when running transcript replay
    ...
    async def estimate_cost(self, request: Request) -> ResourceEstimate:
        raise ReplayExhausted(
            ...
            "to LLMHarness(...) if you intend re-execution."
        )
...
class ReplaySession:
    ...
    into LLMHarness / ToolHarness to drive the re-run.
    ...
    replay_cursor:
        Pass to ``LLMHarness(replay_cursor=...)``. ``None`` when
        re_execute_llm was True — in that case you must pass a real
        adapter.
    tool_replayer:
        Pass to ``ToolHarness(tool_replayer=...)``. Always present (cheap
    ...
    def stub_adapter(self) -> LLMAdapter:
        ...
        real adapter to ``LLMHarness(...)`` instead.
        """
...
def replay(
    ...
    *,
    ...
    re_execute_llm:
        ...
        real LLMAdapter and the LLMHarness is genuinely re-run against the
