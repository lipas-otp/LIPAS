"""LIPAS · Layer 0.5 — SQLite-backed ClaimStore (B1 v0.1).
...
- ``meta`` table tracks ``lipas_schema_version`` (currently 1),
  ...
  open time — there is no migration tooling in v0.1.
...
- Strategy-registry persistence: the registry is *code*, not data.
  ...
  currently an implicit assumption inherited from A1 §replay
  equivalence; reopening with a different registry MAY produce a
  different ``merged`` claim. Documented here, will be made explicit
  in A1 v0.2.
...
from .codec import (CodecRegistry, decode, encode)
from .types import make_default_codec_registry


# Bumped only on incompatible schema change. v0.1 = 1.
SCHEMA_VERSION = 1
...
class SqliteClaimStore:
    ...
    def __init__(
        ...
        *,
        ...
        # check_same_thread=True (the default) is intentional — single
        # writer is part of v0.1's contract; sharing across threads
    ...
    def _load_existing(self) -> None:
        ...
        for (seq, claim_id, tag, kind, priority,
            ...
            # Per-row schema-version check. v0.1 mirrors the meta-level
            # check; v0.2+ may dispatch per-row migrations here.
        ...
        # if a future v0.2 ever rewrites history (none planned).
    ...
    def fold(self, claim: Claim) -> Claim:
        ...
        try:
            ...
            # Most likely: claim_id collision (codec users replaying a
            # log into the same file). Surface clearly.
            raise LipasError(
                f"sqlite fold rejected for claim_id={claim.claim_id!r} "
                f"seq={claim.seq}: {e}"
            ) from e
